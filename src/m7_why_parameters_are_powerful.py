...
def run_test_better_draw_circles():
    ...
    print('--------------------------------------------------')
    draw_better_circles(5)


def draw_better_circles(n):
    window = rg.RoseWindow(400, 400)
...
def run_test_even_better_draw_circles():
    ...
    c = rg.Point(300,200)
    d = .01
    even_better_draw_circles(a, b, c, d)


def even_better_draw_circles(rad, numcirc, center, runtime):
    window = rg.RoseWindow(400, 400)

    for k in range(numcirc + 1):
        circle = rg.Circle(center, rad * k)
        circle.attach_to(window)
        window.render(runtime)  # Pauses for 0.05 seconds after rendering.
